...
def active_language_code():
    try:
        return bpy.context.preferences.view.language or "DEFAULT"
    except Exception:
        return "DEFAULT"


def interface_translation_enabled():
    try:
        view = bpy.context.preferences.view
        return bool(
            getattr(view, "use_translate_interface", False)
            or getattr(view, "use_translate_tooltips", False)
            or getattr(view, "use_translate_new_dataname", False)
        )
    except Exception:
        return False


def should_translate_to_chinese():
    if not interface_translation_enabled():
        return False
    language = active_language_code()
    return language in {"zh_CN", "zh_HANS"} or language.startswith("zh_")


def tr(text):
    if not text:
        return text
    if not should_translate_to_chinese():
        return text
...
def localized_theme_label(label, path):
    if not should_translate_to_chinese():
        return english_theme_label("", path)
...
def localized_section_label(section):
    legacy_labels = {
        "学习记录": "Learned",
        "指针颜色": "Pointer Color",
        "当前区域": "Current Region",
    ...
    return tr(legacy_labels.get(section, section))
...
def populate_candidate_collection(context, candidates=None):
    ...
    for group in build_mode_color_groups():
        ...
        for item in group_items:
            ...
            entry.section = section_by_path.get(item["path"], "")
            entry.number = number
            number += 1
    ...
    for item in candidates:
        ...
        entry.label = label_text
        entry.path = item["path"]
        entry.section = item.get("section", "")
        entry.number = number
        number += 1
    restore_candidate_preview()
...
def learned_candidate_paths():
    ...
    for index, record in enumerate(load_learning_cache()):
        ...
        if score is None:
            continue
        matches.append((score, record.get("last_used", 0.0), index, path))
    matches.sort(key=lambda item: (-item[0], -item[1], item[2]))
    result = []
    seen = set()
    for _score, _last_used, _index, path in matches:
        canonical_path = canonical_theme_path(path)
        if canonical_path in seen:
            continue
        seen.add(canonical_path)
        result.append((canonical_path, semantic_label_for_path(canonical_path)))
    return result


def remember_candidate_choice(context, path):
    if not path:
        return
    key = current_learning_key()
    ...
    existing.update({
        "path": canonical_path,
        "label": semantic_label_for_path(canonical_path),
        "area": key.get("area", ""),
    ...
    def execute(self, context):
        ...
        if getattr(wm, "theme_probe_candidate_preview_index", -1) == self.index:
            ...
            if items and 0 <= self.index < len(items):
                remember_candidate_choice(context, items[self.index].path)
            schedule_candidate_preview(context)
        tag_redraw_all()
        return {"FINISHED"}
...
class THEMEPROBE_UL_candidates(UIList):
    ...
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        ...
        if item.section and item.section != previous_section:
            ...
            header.label(text=localized_section_label(item.section))
        row = column.row(align=False)
        display_label = localized_theme_label(item.label, item.path)
        if owner is None:
            row.label(text=f"{item.number}. {display_label}", icon="ERROR")
            return
        content_row = row
